...
### Final publication-window transaction follow-up

- macOS does not provide an FD-to-name atomic publication primitive. For an existing regular destination, replacement now uses descriptor-relative `renameatx_np(RENAME_SWAP)` as a transaction: the old target remains at the prepared name while the published target is checked no-follow against the validated source identity.
- A deterministic hook runs exactly after that final prepared-entry check and immediately before the swap. If the source is replaced with a symlink (or if either published/displaced entry is not regular), the transaction swaps back, synchronizes the restored directory entries, and returns an unsafe-target error. No recursive deletion is used.
- The exact-window regression proves the old target remains regular and unchanged after a prepared-symlink substitution. Concurrent atomic writes still install one complete payload and leave no temporary files. Focused `AtomicIOTests` passed 22 tests before final full-suite and release verification.
