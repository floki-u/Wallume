...
Settings lifecycle verification recorded 2026-07-20: `swift test` passed 399 tests; release builds of `WallumeApp`, `wallume-runtime`, `wallume-media`, and `wallume-restore` plus `git diff --check` passed. Application termination cancels and awaits an incomplete Settings diagnostics export before retaining the lock-screen → diagnostics → runtime shutdown order; persisted settings remain unchanged. Manual localization, state-restoration, UI acceptance, and real-system Lock Screen acceptance remain pending.
