...
///
/// The extension reads the same file on startup and when it receives `com.wallume.app.prefsChanged`.
...
final class WallpaperPrefsService {
    ...
    var systemWallpaperIsOurs: Bool {
        ...
        /// machine where WallumeProviderHost isn't the active system wallpaper (see ScreenshotGallery).
    ...
    private static var extensionDocsURL: URL {
        FileManager.default.homeDirectoryForCurrentUser
            .appendingPathComponent("Library/Containers/com.wallume.app.wallpaper/Data/Documents")
    }
    ...
    private func observeAgentStuck() {
        ...
        unsafe CFNotificationCenterAddObserver(
            ...
            "com.wallume.app.agentStuck" as CFString,
            nil,
            .deliverImmediately,
        )
    }
    ...
    private func restartStuckAgent() {
        ...
        guard now.timeIntervalSince(lastAgentKill) > 15 else {
            NSLog("[WallumeProviderHost] agentStuck signal ignored (killed agent \(Int(now.timeIntervalSince(lastAgentKill)))s ago)")
            return
        }
        lastAgentKill = now
    ...
    private func killWallpaperAgent(reason: String) {
        NSLog("[WallumeProviderHost] killall WallpaperAgent — \(reason)")
        let task = Process()
    ...
    private func savePrefs() {
        ...
        CFNotificationCenterPostNotification(
            center,
            CFNotificationName("com.wallume.app.prefsChanged" as CFString),
            nil,
            nil,
            true,
        )
    ...
    private func observeStateChanges() {
        ...
        unsafe CFNotificationCenterAddObserver(
            ...
            "com.wallume.app.stateChanged" as CFString,
            nil,
            .deliverImmediately,
        )
    }
    ...
    private static let extensionBundleID = "com.wallume.app.wallpaper"
