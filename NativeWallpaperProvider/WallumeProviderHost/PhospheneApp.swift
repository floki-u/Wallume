...
struct WallumeProviderHostApp: App {
    ...
    @State private var manager: WallumeProviderHostManager

    @Environment(\.openWindow) private var openWindow

    init() {
        let manager = WallumeProviderHostManager()
    ...
    var body: some Scene {
        ...
        Window("WallumeProviderHost", id: "library") {
            LibraryWindow(manager: manager)
        }
        .defaultSize(width: 900, height: 600)
        .commands {
            CommandGroup(replacing: .appInfo) {
                Button("About WallumeProviderHost") {
    ...
    nonisolated func application(_: NSApplication, open urls: [URL]) {
        Task { @MainActor in
            for url in urls {
                ...
                WallumeProviderHostManager.shared?.openVideoChooser()
            }
        }
    }
}
